-- ════════════════════════════════════════════════════════════════════════════
-- Trial balance workflow, part 2 (PR #32; applied together with 20260922180000 and 20260923100000):
--
--   A. USER-BASED VALIDATION. process-trial-balance no longer requires a firm_members row. Whoever may act on
--      the workspace's trial balance (the workspace owner, or an explicit prepare_trial_balance /
--      manage_source_files grant) may validate it. An accepted firm member keeps exactly the access they had
--      before (the pre-existing boundary is preserved, not widened or narrowed).
--      The engine ledgers (engine_runs, idempotency_keys) gain an attributable user actor that is NOT a firm
--      membership: actor_type 'workspace_user' with actor_user_id = the authenticated user. Nothing is
--      fabricated: no firm_members row is created, and a workspace_user row can never carry one.
--
--   B. SCHEDULED SOURCE SWEEPER. Server-only, on a schedule, with no client involvement:
--        * purges discarded sources once the discard is terminal (completed and the undo window is over);
--        * finishes a cancelled replacement's Storage cleanup the browser never completed;
--        * reclaims objects uploaded against a reservation that expired unconsumed (abandoned uploads).
--      pg_cron calls public.tbu_run_source_sweeper(), which mints a single-use ticket and posts it (pg_net) to
--      the trial-balance-source-sweeper Edge Function. The function redeems the ticket, then deletes exactly
--      the objects the database lists, verifies each is gone, and records completion in the database.
--      No key is stored anywhere: the ticket is minted by the database and is valid once, for 5 minutes.
--
-- Nothing here deletes an upload, a certification or any evidence. Every eligibility rule is the SAME one the
-- user-facing RPCs use (tbu_undo_window(), reservation expiry), so the sweeper can never delete a source that
-- could still be restored or registered.
-- ════════════════════════════════════════════════════════════════════════════

-- ── A1. Engine ledgers: an attributable, non-membership user actor ────────────────────────────────
...
ALTER TABLE public.idempotency_keys
  ...
  ADD CONSTRAINT chk_ik_actor_pairing CHECK (
    ...
    (actor_type = 'workspace_user' AND actor_user_id  IS NOT NULL AND firm_member_id IS NULL) OR
    (actor_type = 'system'         AND firm_member_id IS NULL     AND actor_user_id IS NULL)),
  -- Same claim identity as before, with the user actor as one more component. Existing rows keep their
  -- identity (actor_user_id is NULL for every 'user' and 'system' row).
...
-- The existing lifecycle guards freeze every identity column they know about; this one freezes the new one.
...
-- ── A2. Who may validate a workspace's trial balance ─────────────────────────────────────────────
-- For process-trial-balance (service_role only). Returns one row, or none when the user may not validate:
--   * an accepted firm member of the workspace:      actor_type 'user', firm_member_id set (unchanged boundary);
--   * otherwise the owner or a prepare_trial_balance / manage_source_files grant holder:
--                                                     actor_type 'workspace_user', firm_member_id NULL.
-- authority_basis says why (firm_membership | workspace_owner | explicit_capability).
...
-- ── A3. The certification trigger names the engine run's user, whichever actor kind ran it ───────
-- Same body as 20260923100000 except the actor lookup: actor_user_id for a workspace_user run, the member's
-- user for a firm-member run.
...
-- ── B1. Sweeper bookkeeping ───────────────────────────────────────────────────────────────────
-- swept_at: the sweeper confirmed an expired, unconsumed reservation leaves no object behind.
...
-- Grace periods. An expired reservation can never be signed, registered or used for a replacement
-- (expires_at <= now() is refused everywhere); the grace only lets an in-flight transaction that started before
-- expiry finish first. A cancelled replacement is normally cleaned up by the browser at once; the sweeper only
-- finishes it when that did not happen.
...
-- ── B2. What the sweeper may do right now (service_role only) ─────────────────────────────────
-- delete_object is true only for an existing object that no upload row references.
-- A reservation is re-examined for a day after expiry: a signed upload URL can still land an object up to
-- 2 hours after it was signed, and the next sweep reclaims it.
...
-- Records a sweep once the server itself sees the object gone. Outcomes: purged | completed | reclaimed |
-- already_done | not_eligible | storage_cleanup_pending | stale | invalid_request. Re-checks every rule.
...
-- ── B3. Invocation: single-use tickets minted by the database; no stored key ────────────────────
...
-- Where the sweeper function lives. Not a secret; set once per environment with tbu_configure_source_sweeper().
...
-- Outcomes: configured | invalid_url.
...
-- A 244-bit single-use token, valid for 5 minutes. Only its SHA-256 is stored.
...
-- True exactly once per ticket, and only before it expires.
...
-- Called by pg_cron. Outcomes: idle | not_configured | scheduler_unavailable | dispatched.
...
-- ── B4. Schedule (where the platform provides pg_cron and pg_net; a no-op elsewhere) ─────────────
DO $$
BEGIN
...
-- Remove the trial-balance-source-sweeper Edge Function and redeploy the previous process-trial-balance first.
-- SELECT cron.unschedule('trial-balance-source-sweeper');   -- where pg_cron is installed
-- DROP FUNCTION IF EXISTS public.tbu_run_source_sweeper(), public.tbu_redeem_source_sweeper_ticket(text),
--   public.tbu_mint_source_sweeper_ticket(), public.tbu_configure_source_sweeper(text),
--   public.tbu_sweeper_complete(text, uuid), public.tbu_sweeper_candidates(integer),
--   public.tbu_cleanup_sweep_grace(), public.tbu_reservation_sweep_grace(), public.tbu_resolve_processing_actor(uuid, uuid);
-- Restore the 20260923100000 body of tb_certification_drives_upload_lifecycle().
-- The engine-ledger constraints can only return to their previous form once no workspace_user row exists;
-- such rows are real history, so keep actor_user_id, the widened constraints and the immutability triggers.
