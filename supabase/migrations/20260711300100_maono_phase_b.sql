-- ============================================================================
-- MAONO · PHASE B · IRON DOME NUCLEAR DESIGN
-- Intelligence Layer — Cash Flow + AI Insights + Alerts + Tanzania Context
--
-- Tables:
--   1. cashflow_forecasts   13-week rolling projection rows
--   2. maono_insights       Append-only AI-generated insights
--   3. variance_alerts      Append-only threshold-breach alerts
--   4. maono_context        Tanzania knowledge base (injected into AI prompts)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS cashflow_forecasts (
  ...
  forecast_week        DATE NOT NULL,          -- Monday of the week
  week_number          INTEGER NOT NULL,        -- 1–13
  opening_cash         NUMERIC(18,2) NOT NULL DEFAULT 0,

  -- Inflows
  expected_ar_inflows  NUMERIC(18,2) NOT NULL DEFAULT 0,  -- AR collections
  expected_other_inflows NUMERIC(18,2) NOT NULL DEFAULT 0,

  -- Outflows
  expected_ap_outflows NUMERIC(18,2) NOT NULL DEFAULT 0,  -- AP payments
  expected_other_outflows NUMERIC(18,2) NOT NULL DEFAULT 0,

  -- Statutory outflows (Tanzania-specific, broken out for visibility)
  ...
  closing_cash         NUMERIC(18,2),          -- computed by edge function (includes carry-forward)

  -- Risk signal
  risk_flag            TEXT NOT NULL DEFAULT 'ok'
                       CHECK (risk_flag IN ('ok', 'watch', 'critical')),
  risk_reason          TEXT,                   -- plain English reason for flag

  -- Confidence (statutory is certain; AR collection is estimated)
...
-- All AI output is stored here with its full input snapshot.
-- confidence_level is NEVER 'high' unless validation passed AND
-- seasonal_periods_available >= min_periods_high threshold.
...
CREATE TABLE IF NOT EXISTS maono_insights (
  ...
  insight_type            TEXT NOT NULL CHECK (insight_type IN (
    'root_cause',   -- WHY did the variance happen?
    'risk',         -- is this a trend or a one-off?
    'decision',     -- 3 decision paths
    'action'        -- ranked recommended actions
  )),

  -- Which accounts this insight addresses
  subject_account_codes   TEXT[],
  subject_pl_categories   TEXT[],

  -- Iron Dome: full snapshot of all DB rows passed to Claude
  -- Every TZS figure in ai_output must appear in this snapshot
  ...
  -- Confidence (from seasonal_periods_available formula)
  confidence_level        TEXT NOT NULL DEFAULT 'none'
                          CHECK (confidence_level IN ('high','medium','low','none','validation_failed')),

  -- Validation result (Iron Dome: numeric tokens in ai_output verified vs snapshot)
  numeric_validation_passed BOOLEAN NOT NULL DEFAULT FALSE,
  numeric_validation_detail JSONB,   -- {checked: n, failed: [token, ...]}

  -- Tanzania context version used
...
--
-- IRON DOME: APPEND-ONLY.
-- Written by maono-monitor (SECURITY DEFINER). Never by client code directly.
...
CREATE TABLE IF NOT EXISTS variance_alerts (
  ...
  alert_type        TEXT NOT NULL CHECK (alert_type IN (
    'variance_threshold',   -- material variance exceeded threshold
    'cash_critical',        -- closing_cash below critical_days runway
    'cash_watch',           -- closing_cash below warn_days runway
    'tra_risk_signal',      -- pattern matches known TRA audit trigger
    'budget_missing',       -- period has no approved budget
    'trend_deterioration'   -- variance getting worse each period
  )),
  ...
  -- Plain English — no jargon, readable by business personnel
  message           TEXT NOT NULL,
  detail            TEXT,

  -- Acknowledgment (non-blocking — alerts are informational)
...
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  -- Allow only acknowledgment fields to be updated
  IF TG_OP = 'UPDATE' THEN
    IF OLD.run_id != NEW.run_id OR OLD.company_id != NEW.company_id OR
...
-- ── 4. maono_context ─────────────────────────────────────────────────────────
--
-- Tanzania knowledge base injected into every AI prompt.
-- Versioned — context_version tracks which version each insight used.
-- AI output is only as good as this context — maintain it carefully.

CREATE TABLE IF NOT EXISTS maono_context (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  context_key      TEXT NOT NULL UNIQUE,  -- e.g. 'statutory_calendar', 'tra_audit_signals'
  context_version  INTEGER NOT NULL DEFAULT 1,
  title            TEXT NOT NULL,
  content          TEXT NOT NULL,   -- plain text block injected into AI system prompt
  is_active        BOOLEAN NOT NULL DEFAULT TRUE,
  updated_by       UUID REFERENCES auth.users(id),
  updated_at       TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- Seed: Tanzania statutory payment calendar (TRA-verified dates)
...
-- ── Grant execute on gate functions ──────────────────────────────────────────

COMMENT ON TABLE maono_insights IS
  'IRON DOME: Append-only. AI insights stored with full input_snapshot. '
  'numeric_validation_passed=FALSE means some numbers in ai_output could not be '
  'traced to the input_snapshot. These insights are shown with a validation_failed '
  'badge and excluded from CFO/Director views.';

COMMENT ON TABLE maono_context IS
  'Tanzania knowledge base injected into every AI prompt. '
  'context_version tracked per insight row. Update content here when regulations change — '
  'do NOT update AI model training. This is the authoritative source for Tanzania context.';
