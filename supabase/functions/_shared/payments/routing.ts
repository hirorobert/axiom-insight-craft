...
export const FLUTTERWAVE_CAPABILITIES: PaymentProviderCapabilities = {
  ...
  environment: Deno.env.get('FLUTTERWAVE_ENVIRONMENT') === 'production' ? 'production' : 'sandbox',
};

/**
 * Providers configured in this deployment (secrets present). A provider
 * whose adapter exists in code but whose secrets are unset is NOT eligible —
 * routing must never select a provider it cannot actually call.
 */
export function getConfiguredProviders(): PaymentProviderCapabilities[] {
  const providers: PaymentProviderCapabilities[] = [];
  if (Deno.env.get('FLUTTERWAVE_SECRET_KEY') && Deno.env.get('FLUTTERWAVE_WEBHOOK_SECRET')) {
    providers.push(FLUTTERWAVE_CAPABILITIES);
  }
