// ============================================================
// CFO Close — Comparative Assurance (Module F v1.0, 2026-06-30)
// Standards: IAS 1 / IPSAS 1 Comparability, ITA Cap.332
//
// THE implementation of the comparative-period analysis. Served by:
//   POST /functions/v1/comparative-assurance-engine   (canonical)
//   POST /functions/v1/kinga-comparative-engine       (legacy name; a compatibility adapter that serves this same
//                                                      function — no divergent logic, no HTTP hop, no recursion)
// Both entry points import and serve this one handler, so a request is processed exactly once and both
// return identical results for the same authorized caller. The engine only reads (no inserts, updates or upserts),
// so it cannot duplicate writes. Comparative Reporting is included in every plan: there is no entitlement gate here.
//
// PURPOSE:
//   Compares the current fiscal period TB against the prior fiscal period TB for the same company. Produces:
...
//   If prior_period_id is omitted, the engine resolves it from fiscal_periods.prior_period_id automatically.
//
// RESPONSE:
...
export async function handleComparativeAssurance(req: Request): Promise<Response> {
