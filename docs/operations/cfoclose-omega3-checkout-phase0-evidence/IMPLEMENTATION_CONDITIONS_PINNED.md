...
This exact strict discriminated-union logic (Condition 2/3 above) is mirrored, verbatim in behavior, as a disposable PowerShell function `Test-IsSafeFlagResult` in `tools/FlutterwaveEvidenceLib.ps1`. A representative subset of the accept/reject cases in "Required executable tests" above — the three accepted shapes, plus the exact regression case (`flagged:true` carrying a `reason`) and a small sample of other rejections — is run against it in `tools/tests/FlutterwaveEvidenceLib.Tests.ps1` (7 of that suite's tests as of this revision; the suite's total test count has grown substantially since this mirror was first added, as it now also covers the Phase-0 collector's own session-binding, allowlist-projection, and timeline-validation hardening). This mirror makes NO network call and touches NO checkout logic — it exists solely to prove, executably, that this document's own specification is self-consistent (no shape is simultaneously accepted and rejected), which is exactly the property the earlier, corrected draft of this document lacked. It is not itself the real implementation and does not substitute for the real Edge Function code once that is authorized.
