...
**Corrected, this revision (LIVE-PATH executability correction):** the collection tooling this evidence depends on is itself corrected in this revision — `SANDBOX_TEST_PROTOCOL.md` mandates one single, deterministic Flutterwave v3 path, and a safe, TEST-mode-only PowerShell collector (`tools/Invoke-FlutterwaveEvidenceCapture.ps1`, with its logic covered by focused tests in `tools/tests/`, enforced in CI) now exists to run that protocol consistently. **This revision fixes two EXECUTION blockers a prior revision's own tests failed to catch, because those tests exercised helper functions and source text but never the real end-to-end orchestration path:** (1) the collector called `Start-Process` against the API REQUEST endpoint itself, never Flutterwave's actual returned hosted-checkout link (`data.link`) — it never even opened the real payment page — corrected via `Get-HostedCheckoutLink`, which extracts and validates `data.link` (exact host `checkout.flutterwave.com`, HTTPS-only) before anything is opened, with `checkout_opened_utc` recorded only after a successful browser launch; (2) the timestamp allowlist matched broad substrings (`created`, `completed`, `processed`, `expire`/`expiry`) against any key name with no value check, meaning a card's real `expiry` field (e.g. `"09/22"`) would have entered the manifest as if it were a timestamp — corrected to an exact-key allowlist combined with a mandatory independent value-shape check. Also fixed: stage files and the manifest are now write-once (atomic create-new, never overwritable); `Verify` now requires BOTH the provider's `data.id` and `data.tx_ref` to match what was actually requested, not just `tx_ref`; and the full orchestration path (HTTP → link extraction → browser open → observation → verification → finalization) is now driven end-to-end in tests via injected ports, not inferred from passing helper-level tests alone. **The corrected collector is required for any future sandbox evidence submitted against this gate** — evidence gathered by any other means (ad hoc scripts, unredacted manual capture, or an earlier version of this collector) should not be accepted as closure evidence without independently verifying it meets the same link-validation, allowlist-projection, session-binding, transaction-binding, and timeline-validation standard.

**Corrected further, this revision (FINAL PROVENANCE CLOSURE — a fresh independent audit found two additional proven blockers plus one documentation contradiction in the above collector, now also fixed):** (1) the collector calculated a raw-response SHA-256 but never actually retained the raw provider response anywhere, so the hash could not be independently re-verified against anything — the claim of "independently verifiable evidence" did not hold. Corrected: **raw response bytes are retained outside Git**, write-once, under a deterministic session-bound filename, for both `CreateCheckout` and `Verify`; **the exact retained bytes are hashed** (never a reconstructed/reserialized JSON copy); and **finalization re-hashes the retained files**, refusing to write a manifest if either raw file is missing, changed, or otherwise fails independent re-verification. (2) finalization proved only that the three evidence stages agreed with EACH OTHER on which collector produced them, never that this common value matched the collector actually running finalization — so stages produced by one collector version could be silently finalized and mis-attributed to a different one. Corrected: finalization now requires every stage's collector content hash and Git SHA to equal the CURRENTLY EXECUTING collector's own values, failing closed otherwise. (3) `SANDBOX_TEST_PROTOCOL.md` still instructed manually copying any field matching a broad timestamp-shaped substring, contradicting the collector's own hardened exact-key-plus-value allowlist — the prose has been corrected to match the code, not the other way around. As a result: **no sanitized artifact contains raw provider content; collector provenance is verified across every stage and finalization; unknown provider fields remain only in protected raw evidence.**

**No sandbox evidence of any kind has been produced by this or any prior revision. This document does not close the gate, and nothing in this revision authorizes checkout implementation, Supabase access, or a real payment. This revision does not close the Flutterwave timestamp-semantics gate either — it only makes the collector safe enough to execute the controlled sandbox experiment later. The Flutterwave timestamp-semantics gate remains OPEN until the sandbox experiment is executed and reviewed.**
...
## 6. Collector provenance closure — summary (this revision)

Raw response bytes are retained outside Git. The exact retained bytes are hashed. Finalization re-hashes the retained files. No sanitized artifact contains raw provider content. Collector provenance is verified across every stage and finalization. Unknown provider fields remain only in protected raw evidence. **The Flutterwave timestamp-semantics gate remains OPEN until the sandbox experiment is executed and reviewed.**
