...
**Design-correction round 5:** a fifth independent review found round 4's own fixes were incomplete or, in two cases, still contained the exact defect they claimed to have closed — `acquire_checkout_intent_lease`'s staleness sweep still transitioned `PENDING`→`EXPIRED` on `expires_at` alone, and a second, independent occurrence of the identical defect was found in the REAL `commit_verified_commercial_payment` function, quoted verbatim for the first time. Also found and fixed: the "crashed-terminal-attempt sweep" was prose-only; `billing_audit_events` inserts omitted `billing_customer_id`; the claim RPC's composite return shape was wrong; a dual-environment credential architecture was specified where none existed; the hardcoded `ALTER DATABASE` project-URL was removed; Flutterwave's `data.created_at` semantics were qualified; and the retry-count contract was unified.

**Design-correction round 6 (this revision, surgical):** a sixth independent review found that round 5's OWN fix for the exact defect it had just found — the `p_provider_created_at` parameter added to `commit_verified_commercial_payment` — repeated the identical overload-discipline mistake round 5 itself corrected for every OTHER function in this package: it called a signature-changing `CREATE OR REPLACE` "a true replace," when adding a new parameter actually creates a second, co-existing overload, leaving the OLD, wall-clock-vulnerable 12-argument signature fully live and callable. Corrected into the same three-phase (create/cut-over/`REVOKE`+`DROP`) discipline already applied everywhere else, with an executable proof the old signature is gone — `Item 1`. Also found and fixed: `requires_manual_review = true` rows were never excluded from the automatic reconciliation claim or crash-reclaim predicates, meaning a flagged row could be silently re-processed by the very automation that was supposed to have handed it to a human — now excluded unconditionally, with one new audited, admin-only `admin_reopen_reconciliation` RPC as the sole re-entry path — `Item 2`. The webhook path had no answer for discovering `PAYMENT_AFTER_INTENT_EXPIRY` itself (as opposed to via reconciliation) — a webhook has no claim token and must never fabricate one to call the reconciliation-only `finalize_reconciliation_attempt` — closed with a new, narrow, claim-token-free `flag_webhook_payment_requires_manual_review` RPC — `Item 3`. The single-function `WEBHOOK_ENVIRONMENT` env-var design assumed a Supabase deployment capability that does not exist (one source deployed twice under different configuration) — replaced with two genuinely separate, source-controlled wrapper functions, each hardcoding its environment identity as a literal, both calling one shared handler — `Item 4`. A new check requires the loaded intent's own `provider_environment` to match the authenticated webhook route's hardcoded environment BEFORE Gate B or commit, closing a cross-environment corroboration risk Gate A alone does not close — `Item 5`. Flutterwave's `data.created_at` semantics are elevated from a qualified caveat to a named, tracked, explicitly BLOCKING pre-implementation gate (`GATE-FLUTTERWAVE-CREATED-AT-SEMANTICS`) with exact closure criteria — `Item 6`. See `DATA_CONTRACTS.md` §7.0, §7.2, §7.4, §12.4; this document's grant table (§8); `THREAT_AND_FAILURE_MODEL.md`, `ACCEPTANCE_MATRIX.md`, and `IMPLEMENTATION_PLAN.md` for the corresponding scenario, test, and file-slice updates.

**Design-correction round 7 (this revision, surgical):** a seventh independent review found round 6's own acceptance test for `requires_manual_review` asserted a literal contradiction with round 6's own `admin_reopen_reconciliation` RPC — the test required "zero rows" for any `UPDATE ... SET requires_manual_review = false` in the applied SQL, a query `admin_reopen_reconciliation`'s own body would fail outright, since clearing the flag is that RPC's entire purpose. **Corrected canonical invariant:** no AUTOMATIC path ever clears the flag; `admin_reopen_reconciliation` alone may, gated on an authenticated commercial administrator, a non-blank reason, and a `PENDING` flagged intent, durably audited — `Item 1`. Round 6's `flag_webhook_payment_requires_manual_review` had no idempotency guard against an already-flagged intent: a retried or duplicate webhook delivery reaching it a second time would insert a SECOND `billing_audit_events` row falsely claiming a fresh `false→true` transition — corrected to lock the intent, check `requires_manual_review` explicitly, and return `ALREADY_FLAGGED` with no further mutation once already set, guaranteeing exactly one transition and one transition-audit row across any number of sequential or concurrent duplicate calls — `Item 2`. Round 6's shared-handler contract for a `provider_environment` mismatch recorded only the intent-level consequence (`billing_audit_events` via the flag RPC), never the delivery-level outcome — a real, Gate-A-passed webhook delivery's own receipt existed but its processing outcome went unrecorded in `payment_webhook_processing_events`, the table this package's own convention otherwise uses for every Gate-A-adjacent rejection reason. Corrected with a new `PROVIDER_ENVIRONMENT_MISMATCH` value in `chk_pwpe_result` and a mandatory processing-event insert alongside the existing receipt and the flag RPC call — `Item 3`. Round 6's Phase C for the old 12-argument `commit_verified_commercial_payment` signature depended on an unbounded, subjective "full observation window" of log-watching, with no hard gate stopping checkout from going live while both signatures were simultaneously callable — corrected to a bounded, checkout-disabled, fully executable sequence: checkout stays disabled through Phase A/B, every 13-arg caller deploys and is verified, `service_role` is `REVOKE`d from the 12-arg overload with a proven permission-denied failure BEFORE the `DROP`, the `DROP` itself is proven by a zero-row `pg_proc` query, and checkout activation is explicitly prohibited until all of the above pass — `Item 4`. `GATE-FLUTTERWAVE-CREATED-AT-SEMANTICS` is hardened with an explicit permitted/prohibited boundary: Phase-0 provider investigation and disposable sandbox test-harness work may proceed now, solely to gather the gate's own closure evidence; no payment-authority implementation depending on `providerCreatedAt` may be approved, merged, or deployed to any environment until the gate closes; and if the evidence disproves the field's assumed semantics, the expiry comparison must be redesigned around a provably correct field, never reinterpreted by assumption — `Item 5`. See `DATA_CONTRACTS.md` §7.0, §7.2, §6a (new), §7.4; this document's grant table (§8, updated below); `THREAT_AND_FAILURE_MODEL.md`, `ACCEPTANCE_MATRIX.md`, and `IMPLEMENTATION_PLAN.md` for the corresponding scenario, test, and file-slice updates.

**Design-correction round 8 (this revision, surgical — an executability correction, not a redesign):** an eighth independent (Codex) review found round 7's own two new mechanisms were themselves unexecutable or incompletely specified. **`flag_webhook_payment_requires_manual_review`'s reason-code validation, `IF p_reason_code NOT IN (...) THEN`, silently passes a `NULL` reason code** — standard SQL three-valued logic makes `NULL NOT IN (...)` evaluate to `NULL`, never `TRUE`, so the `IF` never branches and a `NULL` call falls through unrejected. Corrected to `IF p_reason_code IS NULL OR p_reason_code NOT IN (...) THEN`, checked before any row is read or locked — `Item 1`. **The round-7 evidence-recording code for a `provider_environment` mismatch, `supabase.from('payment_webhook_processing_events').insert({receipt_id, processing_result})`, targets only two of the real table's columns** — Codex's direct schema read confirms `provider` and `signature_valid` are also `NOT NULL` with no default, so this insert would fail outright the first time it ran, the identical defect class this package's own discipline exists to catch. Corrected to reuse the EXISTING, unchanged `recordProcessingEvent` helper (already used elsewhere in this same handler for Gate A/Gate B evidence), whose return contract is itself corrected to report `{success: boolean}` explicitly rather than silently swallowing a database error. The mismatch branch now: returns HTTP 500 (never 200, never calling the flag RPC) if `recordProcessingEvent` reports failure; checks the flag RPC's own error channel explicitly and returns HTTP 500 on a genuine database error there too; and returns HTTP 200 only once processing evidence is durably recorded AND the flag RPC returned one of its three explicitly-safe outcomes (`FLAGGED`, `ALREADY_FLAGGED`, `INTENT_NOT_PENDING`) — `Item 2`. See `DATA_CONTRACTS.md` §6a, §7.2, §12.4 for the corrected code and the retry-convergence proof; `ACCEPTANCE_MATRIX.md` §12 for the new executable tests (10a–10b, 11a–11f).
...
Flutterwave hosted checkout (outside this system's trust boundary)
  ↓
commercial-payment-webhook-sandbox / commercial-payment-webhook-production (round 6, item 4 — TWO
  genuinely separate Edge Function deployments, replacing round 5's single-function
  Deno.env.get('WEBHOOK_ENVIRONMENT') design, which assumed a Supabase deployment capability that
  does not exist; each is a ~2-line wrapper hardcoding its own environment as a literal, both
  calling ONE shared handleCommercialPaymentWebhook(req, environment), DATA_CONTRACTS.md §12.4) —
  the shared handler's logic is UNCHANGED from round 1 except the Gate B transient/definitive split
  (item 7) and two new steps below:
  1. INSERT payment_webhook_receipts (immutable, BEFORE any verification)
  2. Gate A: adapter.verifyWebhookAuthenticity(rawBody, headers) — adapter is
     getFlutterwaveAdapter(environment), environment fixed by which of the two functions was called
  ...
  4b. NEW (item 5): intent.provider_environment must equal the calling function's own hardcoded
     environment, checked HERE, before Gate B or any commit attempt — Gate A proves the request came
     from Flutterwave's own sandbox/production account, but says nothing about which environment the
     REFERENCED intent itself belongs to; a mismatch calls the EXISTING, unchanged recordProcessingEvent
     helper (corrected round 8, item 2 — round 7's own raw INSERT named only two of the real table's
     columns; the real table also requires provider/signature_valid NOT NULL, so recordProcessingEvent
     is reused instead, its own return contract now corrected to report success/failure explicitly)
     against THIS delivery's own already-inserted receipt; on recordProcessingEvent FAILURE, returns
     HTTP 500 immediately WITHOUT calling the flag RPC and WITHOUT returning 200 (Flutterwave retries);
     on success, calls flag_webhook_payment_requires_manual_review (item 3/5, claim-token-free, and
     idempotent as of round 7 item 2 — a duplicate or concurrent redelivery of the same mismatch
     returns ALREADY_FLAGGED with no second audit row; NULL-reason-code validation corrected round 8,
     item 1), checks ITS error channel explicitly, and returns 200 only once BOTH calls have succeeded
     or returned an explicitly-safe result — never proceeding to Gate B. No payment_events success row,
     no licence mutation, no entitlement mutation, and no commit_verified_commercial_payment call occur
     on this path, regardless of which of the two calls' outcomes was reached.
  ...
  6. authoriseCommit(intent, transaction)                             — 3rd layer; expiry check
     ...
     four-part lifecycle, never to a single rejected commit attempt); the webhook path has no claim
     token, so on this exact rejection it calls flag_webhook_payment_requires_manual_review (item 3,
     round 6) rather than the reconciliation-only finalize_reconciliation_attempt
  7. commit_verified_commercial_payment(...) RPC (service_role, SECURITY DEFINER) — quoted verbatim
     ...
     description. The p_provider_created_at fix is a NEW 13-argument overload, not an in-place
     replace (round 6, item 1 — the identical overload-discipline mistake this design had already
     corrected for every OTHER function); the old 12-argument, wall-clock-vulnerable signature has its
     service_role grant REVOKE'd and is then DROP'd in a later, separate, checkout-GATED Phase C —
     corrected round 7 (item 4) from round 6's unbounded "full observation window" to a bounded
     sequence: checkout stays disabled through Phase A/B, every 13-arg caller deploys and is verified,
     the REVOKE is proven by a failed 12-arg call (permission-denied), the DROP is proven by a
     zero-row pg_proc query, and checkout activation is explicitly prohibited until all of these pass
     (DATA_CONTRACTS.md §7.0). Corrected: idempotency_key check FIRST; SELECT...FOR UPDATE on the intent row;
...
Reconciliation (item 7; round 1 left "Edge Function or pg_cron" unresolved — round 2 committed to
  ...
  auto-classified). Once requires_manual_review=true, BOTH the claim predicate AND the crashed-
  terminal sweep exclude the row unconditionally (item 2, round 6) — the ONLY way it becomes
  automatically claimable again is admin_reopen_reconciliation, an audited, is_commercial_admin()-
  gated RPC. Every billing_audit_events insert along this path supplies the real, required
...
| `commit_verified_commercial_payment(...13 args...)` (round 6, item 1 — a NEW overload, not a replace of the 12-arg form) | **`service_role` only** | Corrected treatment: Phase A creates this overload alongside the still-live 12-arg one; Phase C `REVOKE`s+`DROP`s the 12-arg form once both callers have cut over — see `DATA_CONTRACTS.md` §7.0 for the full three-phase discipline and the executable proof the old signature is gone |
| `admin_reopen_reconciliation(UUID, TEXT)` (new, round 6, item 2) | `authenticated`, `is_commercial_admin()`-gated | The ONE function authorized to clear `requires_manual_review` back to `false` (round 7, item 1's corrected canonical invariant: no AUTOMATIC path ever clears it; this RPC alone may, and only for an authenticated admin, non-blank reason, `PENDING` flagged intent, durably audited) — every other path (claim, crash-reclaim) unconditionally excludes flagged rows rather than clearing the flag itself |
| `flag_webhook_payment_requires_manual_review(UUID, TEXT, TEXT, UUID)` (new, round 6, items 3/5; hardened for idempotency round 7, item 2; NULL-reason validation corrected round 8, item 1) | **`service_role` only** | Claim-token-free escalation for webhook-originated `PAYMENT_AFTER_INTENT_EXPIRY` and `PROVIDER_ENVIRONMENT_MISMATCH` — never requires, accepts, or fabricates a reconciliation claim token; transition-idempotent — locks the intent, returns `ALREADY_FLAGGED` with no mutation if already flagged, permits exactly one `false→true` transition and one transition-audit row regardless of sequential or concurrent duplicate calls; rejects a `NULL` reason code (round 8 — `NOT IN` alone silently passes `NULL` under SQL three-valued logic) with no row read, locked, or mutated |
