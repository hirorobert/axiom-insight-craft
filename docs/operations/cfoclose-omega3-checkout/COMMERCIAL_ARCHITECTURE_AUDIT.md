...
commercial-payment-webhook-sandbox / commercial-payment-webhook-production (round 6, item 4 — TWO
  ...
  4b. NEW (item 5): intent.provider_environment must equal the calling function's own hardcoded
     ...
     REFERENCED intent itself belongs to; a mismatch records a PROVIDER_ENVIRONMENT_MISMATCH row in
     payment_webhook_processing_events against THIS delivery's own already-inserted receipt (item 3,
     round 7 — the delivery-level evidence round 6 omitted), then calls flag_webhook_payment_requires_
     manual_review (item 3/5, claim-token-free, and idempotent as of round 7 item 2 — a duplicate or
     concurrent redelivery of the same mismatch returns ALREADY_FLAGGED with no second audit row) and
     returns 200, never proceeding further. No payment_events success row, no licence mutation, no
     entitlement mutation, and no commit_verified_commercial_payment call occur on this path.
...
| `flag_webhook_payment_requires_manual_review(UUID, TEXT, TEXT, UUID)` (new, round 6, items 3/5; hardened for idempotency round 7, item 2) | **`service_role` only** | Claim-token-free escalation for webhook-originated `PAYMENT_AFTER_INTENT_EXPIRY` and `PROVIDER_ENVIRONMENT_MISMATCH` — never requires, accepts, or fabricates a reconciliation claim token; transition-idempotent — locks the intent, returns `ALREADY_FLAGGED` with no mutation if already flagged, permits exactly one `false→true` transition and one transition-audit row regardless of sequential or concurrent duplicate calls |
