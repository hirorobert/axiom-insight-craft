...
**Design-correction round 4 (this revision):** corrected §3 test 5 ("Expired intent") for the new provider-timestamp-based expiry check (item 3); corrected §1's precondition wording to `admin_set_offer_purchasable`, unchanged in substance from round 3's own correction. New §9 adds twelve executable tests (item 8) for: sandbox/production `provider_environment` isolation on reuse, offer-deactivation-races-acquisition, provider-restriction mismatch, payment-before-expiry-with-delayed-webhook, unpaid-intent-after-expiry, crash-between-claim-and-worker-start, crash-on-the-final-attempt, stale-worker-terminalization-after-success, backoff-from-actual-completion, missing-project-URL, and Vault/Edge-secret-mismatch.
...
| 16 (round 3) | **Bounded client retry on 202** — mock the checkout-creation endpoint to return 202 `CHECKOUT_IN_PROGRESS` three times in a row, then a real 200 | The frontend's `createCheckoutIntent` retries per the `Retry-After` header, succeeds on the 4th observed response (within its 3-retry/15-second budget), and never issues a 4th retry attempt or exceeds the 15-second cap in this scenario |
| 17 (round 3) | **Bounded client retry, exhausted** — mock the endpoint to return 202 indefinitely | The frontend gives up after 3 retries or 15 seconds (whichever first), surfaces a "still processing, try again shortly" message, and issues no further automatic retries |
| 18 (round 3) | **Reconciliation crash recovery at the terminal attempt** — in a staging harness, manually set a `PENDING` intent's `reconciliation_attempt_count = 8` and `reconciliation_lease_expires_at` to a past timestamp (simulating a crash immediately after the 8th claim) | The next `claim_stale_checkout_intents_for_reconciliation` call's `terminal` CTE transitions the row to `FAILED`; a `billing_audit_events` row and a structured log line are produced; the row is never claimed again |
...
| 11 | **Missing project URL** — attempt the `cron.schedule(...)` call (§7.5) against a database where `app.settings.project_url` was never set via `ALTER DATABASE` | The `net.http_post` call fails with a clear "unrecognized configuration parameter" error, caught during the deployment preflight, never silently sending requests to a malformed URL |
...
## 10. Executable migration-replay acceptance (item 12 — new, round 3)
...
## 11. Browser acceptance (1440 / 1024 / 390 px)
...
## 12. What this matrix deliberately does not assert
