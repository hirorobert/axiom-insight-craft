...
**Design-correction round 3 (this revision):** corrected the `LEASE_HELD` contract from a bare 409 to a bounded 202/`Retry-After` client-retry protocol (item 8, tests 6, 16–17); corrected the failed-CAS test's column name to the real `provider_checkout_url` (item 2, test 8); corrected the transient-constraint test to reflect that reconciliation outcomes live in a NEW dedicated table, not `payment_webhook_processing_events` (item 4, test 11); added tests for fail-closed provider-capability resolution (item 9, test 15), reconciliation crash recovery at the terminal attempt (item 5, test 18), `admin_supersede_commercial_offer` product scoping (item 10, test 19), Flutterwave lookup-by-reference determinism (item 11, test 20), and cron-secret verification/rotation (item 7, tests 21–22); and added a new §9 requiring executable migration-replay acceptance against a real Postgres instance, not static source-text checks alone (item 12) — motivated by this same round's own schema-verification findings, which static review had missed.
...
| 4 | **Direct RPC privilege attack — `claim_stale_checkout_intents_for_reconciliation` / `release_reconciliation_lease`** | Same — permission-denied for an authenticated, non-service-role caller for each of the two |
...
## 9. Executable migration-replay acceptance (item 12 — new, round 3)
...
## 10. Browser acceptance (1440 / 1024 / 390 px)
...
## 11. What this matrix deliberately does not assert
