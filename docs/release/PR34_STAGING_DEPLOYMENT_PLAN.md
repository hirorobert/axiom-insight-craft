...
- That function is the ONLY creation path: anon, authenticated and service_role hold no INSERT / UPDATE / DELETE on
  `deployment_approvals`; an insert guard refuses any row not written by the function in the same transaction; the
  approver must be the calling, active commercial administrator (`approved_by` references `commercial_admins(user_id)`).
- Scope: a true database superuser (or the table owner acting outside the function) is outside every application-level
  control — it can disable triggers or replicate rows. Restricting who holds superuser credentials on the hosted project
  is an operational control, not something a migration can enforce.
...
5. `seal-reporting-pack` (new): the official Reporting Pack is generated by the database; the function stores the
   database's bytes in the private `reporting-packs` bucket and seals them. It accepts no uploaded bytes.
6. The frontend (Lovable), because the export sites call `issue_reporting_pack` and the official path calls
   `seal-reporting-pack`.
