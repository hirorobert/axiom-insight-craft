...
**Ω3 additions — three, not guessed, across two design passes:** `'ADMIN'`, `'PRODUCT'` (original Ω3.0 draft), and `'PLATFORM_STATE'` (added in the Ω3.0 code-audit remediation round, Finding 1 — see the correction immediately below).

**Final complete allowed vocabulary after this design:** `'OFFER'`, `'PLAN'`, `'ADMIN'`, `'PRODUCT'`, `'PLATFORM_STATE'` — five values, two pre-existing (one previously unused), three new.

**Corrected — `admin_transition_platform_state` does NOT write `entity_type='ADMIN'` (code-audit Finding 1, BLOCKER).** The original draft had `admin_transition_platform_state` write `entity_type='ADMIN', entity_id=<the acting administrator's own user id>`. This was wrong on both fields at once: `actor_user_id` already identifies the administrator, so repeating that same UUID as `entity_id` identifies the actor twice and never identifies *what was actually mutated* — the singleton platform-state authority itself. The corrected design establishes a dedicated `entity_type='PLATFORM_STATE'` and a **canonical, constant, deterministic entity UUID for the singleton authority itself** — never an administrator's id, never generated per request, never dependent on the current state or deployment time:

```
c_platform_state_entity_id := UUIDv5(NAMESPACE_URL, 'saff:commercial_platform_state:singleton:v1')
                             = 4056be2d-92cb-56eb-9ef4-c11e13579b94
```

Computed once, offline, per RFC 4122 §4.3 (namespace = the standard `NAMESPACE_URL`, `6ba7b811-9dad-11d1-80b4-00c04fd430c8`) and embedded as a literal `CONSTANT UUID` inside `admin_transition_platform_state`'s own `DECLARE` block — no new extension, no runtime UUID generation, identical across every environment and every transition. Cross-verified against two independent RFC-4122 v5 implementations (Node.js manual SHA-1 construction and Python's `uuid.uuid5`), both producing the identical value. Every transition, by every administrator, writes the same `entity_id`; only `actor_user_id` varies. This makes the event structurally impossible to mistake for an `'ADMIN'`-entity mutation (a different `entity_type` value entirely) and impossible to misread as "the administrator is the thing that changed."
...
| `'ADMIN'` | `commercial-admin-bootstrap` Edge Function (new, via its `service_role` client) — `FOUNDER_BOOTSTRAP`; `admin_grant_commercial_admin` (new) — `ADMIN_GRANTED`; `admin_revoke_commercial_admin` (new) — `ADMIN_REVOKED` |
| `'PRODUCT'` | `admin_upsert_commercial_product` (new) — `PRODUCT_CREATED`/`PRODUCT_UPDATED` |
| `'PLATFORM_STATE'` | `admin_transition_platform_state` (new) — `PLATFORM_STATE_TRANSITIONED`, `entity_id` always the canonical constant above, never an administrator's id |
...
**Idempotency mechanism:** key = the existing `offer_code` column (no new key column); uniqueness boundary = the existing `uq_co_offer_code` constraint (no new constraint); scope = table-global; fingerprint = a new `request_fingerprint TEXT NULL` column, computed via `pgcrypto`'s `digest()` over a **versioned JSONB payload (contract v1)** built from the 9 semantic parameters (`p_old_offer_code`, `p_plan_code`, `p_market_code`, `p_currency_code`, `p_billing_interval`, `p_billing_interval_count`, `p_amount_minor`, `p_currency_exponent`, `p_effective_start`), excluding the key itself (`p_new_offer_code`) and the free-text `p_reason` — unchanged in *scope* from the prior pass, but the *mechanism* is corrected in this pass (code-audit Finding 2). The prior mechanism (`concat_ws('|', ...)` over `::TEXT`-cast parameters) is replaced because it had three independent defects: `concat_ws` silently drops `NULL` positions (conflating `NULL` with an empty string, and conflating "this parameter is `NULL`" with "this parameter is missing from the payload entirely"), a `|`-delimited join has no structural protection against a `|` character appearing inside a text parameter's own value, and `p_effective_start::TEXT` depends on the calling session's `TimeZone` GUC — the identical instant could hash differently depending on which connection computed it. The corrected mechanism:

- Uses `jsonb_build_object('v', 1, ...)` — an explicit fingerprint-contract version, structural (not textual) field boundaries via JSON's own escaping (a `|`, a quote, a backslash, or arbitrary Unicode inside any parameter can never be misread as a field boundary), and a genuine JSON `null` for a `NULL` parameter (`p_old_offer_code` is `NULL` exactly when opening a brand-new family) — never conflated with `""` or with the field being absent.
- Renders `p_effective_start` via `to_char(p_effective_start AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"')` — `AT TIME ZONE 'UTC'` normalizes the instant before formatting, so the identical instant hashes identically regardless of the calling session's `TimeZone` setting; the format template is numeric-only (no month/day names), so it is also independent of the session's `lc_time`/`DateStyle`; microsecond precision (`.US`) means two `effective_start` values one microsecond apart hash differently; the field carries two independent, unambiguous UTC markers — a literal trailing `"Z"` embedded in the formatted value itself, and the `_utc` suffix on the field name itself.
- Converts the JSONB payload's `::TEXT` serialization to `bytea` via an **explicit** `convert_to(..., 'UTF8')` before `digest()` — not an implicit text→bytea cast that would otherwise depend on the database's own server/client encoding setting.
- `amount_minor`/`currency_exponent`/`billing_interval_count` are emitted as genuine JSON numbers (jsonb's own fixed, locale-independent numeric serialization — no thousands separators, no decimal comma), never as locale-formatted text.
- `jsonb_build_object`'s own on-disk key ordering is by (length, then lexicographic value) — not call-site order — so the resulting `::TEXT` serialization is already canonical and deterministic regardless of how the object literal happens to be written in the function body.
...
BEGIN
  ...
  -- STEP 3: calculate the canonical request fingerprint (contract v1 —
  -- corrected, code-audit Finding 2: a versioned, structured JSONB
  -- payload, never delimiter concatenation; see the discussion
  -- immediately after this function's listing for the full rationale).
...
`admin_transition_platform_state(p_new_state TEXT, p_reason TEXT)` — `is_commercial_admin()`-gated, audited (`commercial_catalog_audit_events`, `entity_type='PLATFORM_STATE'`, `entity_id`=the canonical singleton constant, §D.1 — never the acting administrator's own id). Full privilege specification in §Q's privilege matrix.
...
- **Fingerprint computation, exact (code-audit Finding 2, versioned JSONB contract v1, corrected from a prior `concat_ws`-based mechanism):** `encode(digest(convert_to(jsonb_build_object('v', 1, 'old_offer_code', p_old_offer_code, 'plan_code', p_plan_code, 'market_code', p_market_code, 'currency_code', p_currency_code, 'billing_interval', p_billing_interval, 'billing_interval_count', p_billing_interval_count, 'amount_minor', p_amount_minor, 'currency_exponent', p_currency_exponent, 'effective_start_utc', to_char(p_effective_start AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'))::TEXT, 'UTF8'), 'sha256'), 'hex')` — exactly 9 semantic parameters, computed server-side via `pgcrypto`'s `digest()` over an explicitly UTF-8-encoded, versioned JSONB payload. `p_new_offer_code` (the key itself) and `p_reason` (free-text metadata) are excluded by design — the key is not part of its own payload, and a differing justification for an otherwise-identical economic request is not a differing request.
...
§E.2/§G.2/§Q/§R/§S/Error Code Reference/Final Summary this pass:
  ...
  CHECK-vocabulary widening:             1   (commercial_catalog_audit_events.entity_type:
                                              'OFFER','PLAN' → 'OFFER','PLAN','ADMIN','PRODUCT','PLATFORM_STATE' —
                                              PLATFORM_STATE added in the code-audit remediation round, Finding 1,
                                              alongside admin_transition_platform_state's canonical singleton entity id)
