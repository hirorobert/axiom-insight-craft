...
interface ExportStatementsProps {
  ...
  /** Workspace and fiscal year the Reporting Pack is issued for (issue_reporting_pack). */
...
export function ExportStatements({
  ...
  const exportToPDF = (packReference: string) => {
    ...
    const stampHeader = (y: number): number => {
      ...
      doc.text(`Generated: ${generatedAt} · ${totalCount > 0 ? `${autoCount}/${totalCount} accounts auto-classified` : "—"} · CFO Close`, 14, y); y += 4.5;
      doc.text(`Issue reference: ${packReference} (working copy, not a sealed official Reporting Pack)`, 14, y); y += 4.5;
      return y + 2;
    };
    ...
    const note3Lines = doc.splitTextToSize(
      ...
      `impairment of receivables, and income tax provisions. These statements were computer-generated by CFO Close from the entity's trial balance ` +
    ...
    if (totalCount > 0) {
      ...
      const note4Lines = doc.splitTextToSize(
        ...
        (autoCount > 0
          ? `${autoCount} accounts (${Math.round((autoCount / totalCount) * 100)}%) were classified automatically by CFO Close's six-tier classifier. `
          : "") +
        (corrCount > 0
  ...
  const exportToExcel = (packReference: string) => {
    ...
    const summaryData = [
      ...
      ["Generated by",        "CFO Close — for professional review before submission"],
      ["Issue reference (working copy, not a sealed official Reporting Pack)", packReference],
      [""],
  ...
  // and its reference is printed in the document. The plan check below only explains; the server decides.
  const { state: commercial, loading: commercialLoading } = useWorkspaceCommercialState(companyId);
  const pack = paidActionState(commercial, "REPORTING_PACK_EXPORT", commercialLoading);
  const [refused, setRefused] = useState(false);
  const locked = refused || pack.status === "locked";

  // Issued by the server (entitlement and capability), built with the issue reference printed in it, and saved as a
  // WORKING COPY (deliverReportingPack): a browser-rendered file is never sealed or official.
  ...
  if (locked) {
    return <PaidActionNotice copy={lockedCopy("REPORTING_PACK_EXPORT")} compact testId="reporting-pack-locked" />;
  }
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem onClick={() => void issueAndExport("financial_statements_pdf", exportToPDF)} className="gap-2 cursor-pointer">
          <FileText className="w-4 h-4" />
          Download as PDF
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => void issueAndExport("financial_statements_spreadsheet", exportToExcel)} className="gap-2 cursor-pointer">
