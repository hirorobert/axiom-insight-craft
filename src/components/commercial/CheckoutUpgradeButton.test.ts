...
describe("Test 3 (static): the client can never override amount/currency/price", () => {
  it("handleUpgrade calls createCheckoutIntent with only planCode — no amount/currency/price argument, even though the component legitimately DISPLAYS the server-resolved offer's amount/currency elsewhere", () => {
    // The component never constructs its own request body — it delegates
    // entirely to createCheckoutIntent(planCode). Scoped to the call site
    // itself (not the whole file) because the file legitimately reads
    // amount_minor/currency_code from the server response to DISPLAY the
    // resolved price — that is presentation, not a client-supplied value.
    expect(SRC).toMatch(/createCheckoutIntent\(planCode\)/);
    const callSite = SRC.match(/await createCheckoutIntent\(([^)]*)\)/);
    expect(callSite).not.toBeNull();
    expect(callSite![1].trim()).toBe("planCode");
  });
...
describe("Test 4 (static): checkout uses the existing server authority, not a new endpoint", () => {
  it("handleUpgrade calls createCheckoutIntent — the same client function already wired to commercial-create-checkout", () => {
    expect(SRC).toMatch(/const \{ data, error \} = await createCheckoutIntent\(planCode\);/);
  });
