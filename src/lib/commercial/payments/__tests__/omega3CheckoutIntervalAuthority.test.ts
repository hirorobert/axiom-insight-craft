...
describe("Ω3-CHECKOUT migration — presence and ordering", () => {
  ...
  it("sorts after every prior live migration (including the Ω3.0 platform-state migration) and before the Ω∞ A+ closure migration that builds on it", () => {
    ...
    expect(thisIndex).toBe(sorted.length - 22);
    ...
    expect(sorted[sorted.length - 14]).toBe("20260919100000_financial_statements_rollout_control.sql");
    expect(sorted[sorted.length - 9]).toBe("20260922180000_discard_trial_balance_authority.sql");
    expect(sorted[sorted.length - 8]).toBe("20260923100000_upload_lifecycle_retire_and_replace.sql");
    expect(sorted[sorted.length - 7]).toBe("20260923120000_workspace_user_engine_actor_and_source_sweeper.sql");
    expect(sorted[sorted.length - 6]).toBe("20260923130000_workspace_capability_access_bridge.sql");
    expect(sorted[sorted.length - 5]).toBe("20260923140000_upload_lifecycle_hardening.sql");
    expect(sorted[sorted.length - 4]).toBe("20260923150000_upload_pointer_and_source_binding.sql");
    expect(sorted[sorted.length - 3]).toBe("20260923160000_personal_upload_lifecycle_audit.sql");
    expect(sorted[sorted.length - 2]).toBe("20260923170000_upload_binding_and_personal_authority.sql");
    // 20260925100000 (CFO Close capabilities, entitlements and pricing catalogue) is the newest entry.
    expect(sorted[sorted.length - 1]).toBe("20260925100000_global_capabilities_entitlements_pricing.sql");
