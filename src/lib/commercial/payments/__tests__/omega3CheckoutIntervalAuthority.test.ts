...
describe("Ω3-CHECKOUT migration — presence and ordering", () => {
  ...
  it("sorts after every prior live migration (including the Ω3.0 platform-state migration) and before the Ω∞ A+ closure migration that builds on it", () => {
    ...
    // 20260922180000 (discard-trial-balance authority), 20260923100000 (upload-lifecycle retire-and-replace) , 20260923120000 (user-based validation + source sweeper) and 20260923130000 (workspace access bridge) and 20260923140000 (upload-lifecycle hardening) are the five newest entries, sorting after all of the above.
    expect(thisIndex).toBe(sorted.length - 18);
    ...
    expect(sorted[sorted.length - 13]).toBe(
    ...
    expect(sorted[sorted.length - 10]).toBe("20260919100000_financial_statements_rollout_control.sql");
    expect(sorted[sorted.length - 5]).toBe("20260922180000_discard_trial_balance_authority.sql");
    expect(sorted[sorted.length - 4]).toBe("20260923100000_upload_lifecycle_retire_and_replace.sql");
    expect(sorted[sorted.length - 3]).toBe("20260923120000_workspace_user_engine_actor_and_source_sweeper.sql");
    expect(sorted[sorted.length - 2]).toBe("20260923130000_workspace_capability_access_bridge.sql");
    expect(sorted[sorted.length - 1]).toBe("20260923140000_upload_lifecycle_hardening.sql");
