...
describe("Ω3-CHECKOUT migration — presence and ordering", () => {
  ...
  it("sorts after every prior live migration (including the Ω3.0 platform-state migration) and before the Ω∞ A+ closure migration that builds on it", () => {
    ...
    expect(sorted[sorted.length - 20]).toBe("20260919100000_financial_statements_rollout_control.sql");
    expect(sorted[sorted.length - 15]).toBe("20260922180000_discard_trial_balance_authority.sql");
    expect(sorted[sorted.length - 14]).toBe("20260923100000_upload_lifecycle_retire_and_replace.sql");
    expect(sorted[sorted.length - 13]).toBe("20260923120000_workspace_user_engine_actor_and_source_sweeper.sql");
    expect(sorted[sorted.length - 12]).toBe("20260923130000_workspace_capability_access_bridge.sql");
    expect(sorted[sorted.length - 11]).toBe("20260923140000_upload_lifecycle_hardening.sql");
    expect(sorted[sorted.length - 10]).toBe("20260923150000_upload_pointer_and_source_binding.sql");
    expect(sorted[sorted.length - 9]).toBe("20260923160000_personal_upload_lifecycle_audit.sql");
    expect(sorted[sorted.length - 8]).toBe("20260923170000_upload_binding_and_personal_authority.sql");
    // 20260925100000 (CFO Close capabilities, entitlements and pricing catalogue), then
    // 20260925110000 (named-user suspension, invitation reservations) and 20260925120000 (official Reporting Pack), then
    // 20260925130000 (Solo, no free plan, plan x capability matrix), 20260925140000 (capability authorization) and
    // 20260925150000 (the minimum grant on the workspace-authority predicates).
    expect(sorted[sorted.length - 7]).toBe("20260925100000_global_capabilities_entitlements_pricing.sql");
    expect(sorted[sorted.length - 6]).toBe("20260925110000_named_user_billing_suspension_and_invitation_lifecycle.sql");
    expect(sorted[sorted.length - 5]).toBe("20260925120000_reporting_pack_issuance_binding.sql");
    expect(sorted[sorted.length - 4]).toBe("20260925130000_solo_plan_no_free_plan_and_plan_feature_matrix.sql");
    expect(sorted[sorted.length - 3]).toBe("20260925140000_workspace_capability_authorization.sql");
    expect(sorted[sorted.length - 2]).toBe("20260925150000_can_user_act_on_workspace_minimum_grant.sql");
    // 20260926160000 (processing entitlement wall): a NEW forward migration after the applied PR #34 chain.
    expect(sorted[sorted.length - 1]).toBe("20260926160000_trial_balance_processing_entitlement_wall.sql");
...
describe("CFOClose Professional offer seeding — GLOBAL/USD, exact economics, non-purchasable", () => {
  ...
  it("seeded the historical $49 / $499 offers (4900 / 49900 minor units), which 20260925100000 retires without deleting", () => {
    // Historical, applied migration: its seeds are unchanged. The current catalogue (Practice $99/$990, Firm
    // $299/$2,990) lives in src/lib/commercial/pricingCatalogue.ts; no public $49 plan remains (pricingCatalogue.test.ts).
    expect(migrationCode).toMatch(/4900/);
    expect(migrationCode).toMatch(/49900/);
    const retirement = fs.readFileSync(
      path.join(REPO_ROOT, "supabase/migrations/20260925100000_global_capabilities_entitlements_pricing.sql"),
      "utf-8",
    );
    expect(retirement).toMatch(
      /SET is_active = false, is_purchasable = false, effective_end = [^\n]*\n WHERE offer_code IN \('CFOCLOSE_PROFESSIONAL_GLOBAL_USD_MONTHLY', 'CFOCLOSE_PROFESSIONAL_GLOBAL_USD_ANNUAL'\)/,
    );
    expect(retirement).not.toMatch(/DELETE FROM public\.commercial_offers/);
  });
