...
export function deriveNextAction(input: { readonly structure: StructureModel; readonly views: readonly FindingView[]; readonly hasReport: boolean }): NextAction {
  ...
  if (codes.has("FRAMEWORK_UNSUPPORTED_FROM_TRIAL_BALANCE")) {
    return { stage: "structure", label: "Review framework limits", reason: "This framework's primary statement cannot be prepared from a trial balance." };
  }
  if (codes.has("FRAMEWORK_NOT_SET") || codes.has("CURRENCY_NOT_SET") || codes.has("PERIOD_BASIS_ASSUMED")) {
