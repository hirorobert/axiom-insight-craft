...
function evaluatePeriod(
  ...
  const discriminator = `${cashFlowStatement.statementId}:${period.periodId}`;
  const affected = { statementId: cashFlowStatement.statementId };

  const closingFactId = closingLine ? (period.isComparative ? closingLine.comparativeFactId : closingLine.currentFactId) : null;
  const sfpFactId = sfpLine ? (period.isComparative ? sfpLine.comparativeFactId : sfpLine.currentFactId) : null;

  if (!sfpStatement) {
    if (period.isComparative) return null;
    return {
  ...
  if (!closingLine || !sfpLine) {
    if (period.isComparative) return null; // only report the comparative gap when a comparative closing line actually exists
    return {
      outcome: "INSUFFICIENT_EVIDENCE",
      severity: "MEDIUM",
      observedValues: {},
      expectedRelationship: "cash flow statement closing cash = SFP cash and cash equivalents",
      deterministicCalculation: !closingLine ? "no cash-flow closing-cash line found" : "no SFP cash-and-cash-equivalents line found",
      evidenceReferences: [],
      affected,
      remediationGuidance: "Declare both the cash-flow closing-cash line and the SFP cash-and-cash-equivalents line with their canonical concepts.",
      discriminator,
    };
  }
  ...
  if (closing.status !== "PRESENT" || sfpCash.status !== "PRESENT") {
    return {
      ...
      expectedRelationship: "cash flow statement closing cash = SFP cash and cash equivalents",
      deterministicCalculation: "one or both figures are missing",
      evidenceReferences: [],
      affected,
  ...
  return {
    ...
    evidenceReferences: [
      { evidenceReferenceId: `${discriminator}:closing`, factId: closingFactId ?? undefined },
      { evidenceReferenceId: `${discriminator}:sfp`, factId: sfpFactId ?? undefined },
    ],
    affected,
    remediationGuidance: pass ? "No action required." : "Reconcile the cash flow statement's closing cash against the SFP's cash and cash equivalents.",
    discriminator,
  };
}
...
export const cashFlowClosingReconciliationRule: RuleDefinition = {
  ...
  evaluate(ctx: RuleContext): readonly RuleEvaluationResult[] {
    ...
    for (const cashFlowStatement of cashFlowStatements) {
      const current = evaluatePeriod(ctx, cashFlowStatement, sfpStatement, { periodId: ctx.report.period.periodId, isComparative: false });
      if (current) results.push(current);
      for (const comparative of ctx.report.comparativePeriods) {
        const result = evaluatePeriod(ctx, cashFlowStatement, sfpStatement, { periodId: comparative.periodId, isComparative: true });
