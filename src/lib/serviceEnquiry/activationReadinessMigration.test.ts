...
describe("readiness migration hygiene", () => {
  it("is timestamped, sorts before only the later, unrelated discard-authority migration, and leaves the original enquiry migration untouched", () => {
    ...
    expect(all[all.length - 1]).toBe("20260923160000_personal_upload_lifecycle_audit.sql");
    expect(all[all.length - 2]).toBe("20260923150000_upload_pointer_and_source_binding.sql");
    expect(all[all.length - 3]).toBe("20260923140000_upload_lifecycle_hardening.sql");
    expect(all[all.length - 4]).toBe("20260923130000_workspace_capability_access_bridge.sql");
    expect(all[all.length - 5]).toBe("20260923120000_workspace_user_engine_actor_and_source_sweeper.sql");
    expect(all[all.length - 6]).toBe("20260923100000_upload_lifecycle_retire_and_replace.sql");
    expect(all[all.length - 7]).toBe("20260922180000_discard_trial_balance_authority.sql");
    expect(all[all.length - 8]).toBe(FILE);
    expect(all[all.length - 9]).toBe("20260921100000_service_enquiry_intake.sql");
