...
describe("readiness migration hygiene", () => {
  it("is timestamped, sorts before only the later, unrelated discard-authority migration, and leaves the original enquiry migration untouched", () => {
    ...
    // 20260925110000_named_user_billing_suspension_and_invitation_lifecycle.sql and 20260925120000_reporting_pack_issuance_binding.sql (named-user suspension, invitation reservations, official Reporting Pack) are later still and equally unrelated.
    // 20260925130000_solo_plan_no_free_plan_and_plan_feature_matrix.sql, 20260925140000_workspace_capability_authorization.sql and 20260925150000_can_user_act_on_workspace_minimum_grant.sql (Solo / no free plan, capability authorization, minimum grant) are later still and equally unrelated.
    // 20260926160000_trial_balance_processing_entitlement_wall.sql (processing entitlement wall) is a NEW forward migration after the applied chain, equally unrelated.
    expect(all[all.length - 1]).toBe("20260926160000_trial_balance_processing_entitlement_wall.sql");
    expect(all[all.length - 2]).toBe("20260925150000_can_user_act_on_workspace_minimum_grant.sql");
    expect(all[all.length - 3]).toBe("20260925140000_workspace_capability_authorization.sql");
    expect(all[all.length - 4]).toBe("20260925130000_solo_plan_no_free_plan_and_plan_feature_matrix.sql");
    expect(all[all.length - 5]).toBe("20260925120000_reporting_pack_issuance_binding.sql");
    expect(all[all.length - 6]).toBe("20260925110000_named_user_billing_suspension_and_invitation_lifecycle.sql");
    expect(all[all.length - 7]).toBe("20260925100000_global_capabilities_entitlements_pricing.sql");
    expect(all[all.length - 8]).toBe("20260923170000_upload_binding_and_personal_authority.sql");
    expect(all[all.length - 9]).toBe("20260923160000_personal_upload_lifecycle_audit.sql");
    expect(all[all.length - 10]).toBe("20260923150000_upload_pointer_and_source_binding.sql");
    expect(all[all.length - 11]).toBe("20260923140000_upload_lifecycle_hardening.sql");
    expect(all[all.length - 12]).toBe("20260923130000_workspace_capability_access_bridge.sql");
    expect(all[all.length - 13]).toBe("20260923120000_workspace_user_engine_actor_and_source_sweeper.sql");
    expect(all[all.length - 14]).toBe("20260923100000_upload_lifecycle_retire_and_replace.sql");
    expect(all[all.length - 15]).toBe("20260922180000_discard_trial_balance_authority.sql");
    expect(all[all.length - 16]).toBe(FILE);
    expect(all[all.length - 17]).toBe("20260921100000_service_enquiry_intake.sql");
