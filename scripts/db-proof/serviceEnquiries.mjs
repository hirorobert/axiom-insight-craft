...
async function main() {
  ...
  // Named-user seats (20260925100000): Company A has one other member, so the account holds a Practice licence with 1 purchased
  // additional named-user seat, recorded on the licence exactly as a real account's would be.
  if ((await admin.query("SELECT to_regclass('public.commercial_capabilities') IS NOT NULL AS ok")).rows[0].ok) {
    ...
    await admin.query("INSERT INTO public.commercial_licences (billing_customer_id, plan_id, status, source, effective_start, additional_seats) SELECT $1, id, 'ACTIVE', 'ADMIN_GRANT', now() - interval '1 day', 1 FROM public.commercial_plans WHERE product_id=$2 AND code='PRACTICE'", [bc, prod]);
  }
